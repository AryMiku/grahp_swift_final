...
class ViewController: UIViewController,ChartViewDelegate {
    ...
    @IBAction func OKNA(_ sender: Any) {
        textdata = Inputcountry.text ?? "asia"
        self.getdata()
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            self.setChart(xValues: self.nameless, yValuesLineChart:  self.country, yValuesBarChart: self.population)
        }
        print(name)
        print(country)
    }
...
//        }
    ...
    func getdata(){
        ...
        self.country = []
        self.nameless = []
        let jsonUrlString = "https://restcountries.eu/rest/v2/region/\(textdata)"
            ...
            do {
                ...
                for mainarr in self.testNa{
                    //if mainarr.subregion == "Southern Asia" {
                        ...
                        self.nameless.append(mainarr.alpha3Code)
                    //}
                }
            } catch let jsonErr {
